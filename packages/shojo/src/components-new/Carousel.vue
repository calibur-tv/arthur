...
<template>
  <div id="v-carousel">
    <VSwitcher
      ...
      @change="handleChange"
    >
      <template v-for="(item, index) in list" #[index] :key="index">
        <a :href="convertUrl(item.link)" :ping="convertPing(item.id)" target="_blank" class="carousel-item">
          <VImg :src="$resizeImage(item.image, { width: 440, height: 220 })" />
        </a>
      </template>
      <template #header-before>
        <a class="title" target="_blank" :href="showUrl" v-text="showTitle" />
      </template>
